...
On intégre ce système avec un pas $\Delta\varphi = 0{,}01\,\text{rad}$.
...
\begin{itemize}
    ...
    \item $r \geq 60\,r_s$: évasion vers l'infini $\rightarrow$ couleur du fond.
...
L'ancienne méthode (changement de signe y, équation $y_{n-1} \cdot y_n < 0$) est abandonnée au profit
d'un disque volumétrique: à chaque pas RK4, le rayon accumule une contribution du disque pondérée par
une gaussienne verticale:

\begin{equation}
    H(r) = 0{,}03 \cdot r \qquad \text{(hauteur d'échelle)}
\end{equation}

\begin{equation}
    w = \exp!\left(-\frac{y^2}{2H^2}\right) \cdot \Delta\varphi \cdot 10
\end{equation}
...
Les bords du disque sont fondus via des fonctions \textit{smoothstep} pour éviter les discontinuités:

\begin{equation}
    \alpha = \text{smoothstep}(r_{\text{outer}}, r_{\text{outer}}-2, r) \cdot \text{smoothstep}(r_{\text{inner}}, r_{\text{inner}}+1{,}5, r)
\end{equation}
...
L'intensité spécifique observée scale en $D^{1,5}$ (\textbf{beaming relativiste}).
...
\begin{equation}
    \boxed{I_{\text{obs}} = D^{1{,}5} \cdot g \cdot I_{\text{base}}(r)}
\end{equation}

L'intensité n'est plus calculée à un seul point d'intersection mais par \textbf{alpha-compositing} le long du rayon:

\begin{equation}
    \text{rgb} \mathrel{+}= I_{\text{obs}} \cdot w \cdot (1 - \text{opacité})
\end{equation}

\begin{equation}
    \text{opacité} \mathrel{+}= w \cdot (1 - \text{opacité})
\end{equation}
